...
export default function UpdatedMaterialsPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!formData.name || !formData.category || !formData.costPerUnit) {
  ...
  const getStockStatus = (material: UserMaterial) => {
    if (!material.inStock) return { status: 'Out of Stock', color: 'text-red-600' }
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="flex items-center">
                    ...
                    />
                    <label htmlFor="inStock" className="ml-2 block text-sm text-gray-700 cursor-pointer">
                      Currently in stock
                    </label>
                  </div>
                </div>
